...
func RunAndPrintResult(sql string, name string) {
	elasticSQL, err := ElasticSQL(sql, "")
	if err != nil {
		fmt.Println("error ", err)
		return
	...
	elasticSQL.Init()
	elasticSQL.Execute()
	fmt.Println(common.JSONPrettyStr(elasticSQL.GetResultSet(name)))
}
...
func TestFullEextMatchPhrasePinyin(t *testing.T) {
	sql := "SELECT * FROM suggest_v4  where match_phrase(field='keyword.keyword_pinyin', query='zhanshi')"
	RunAndPrintTheResult(sql)
}
...
func TestNested4(t *testing.T) {
	sql := "select mid from wb_mdoc_online where uid='1989660417' and create_time>'now-30d' and new_adjust_content_tag$keyword='疫情'   order by create_time desc "
	RunAndPrintTheResultInSomeCluster(sql, "elastic-sql-rest-g2.yml")
}
...
func TestNowTime(t *testing.T) {
	sql := "SELECT * FROM simba* where  match(field='title', query='NBA') and ctime> 'now-1d' order by ctime desc"
	RunAndPrintTheResultInSomeCluster(sql, "elastic-sql-rest-g2.yml")
}
